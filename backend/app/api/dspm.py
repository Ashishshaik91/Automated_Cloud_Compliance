...
Threat intel fields surfaced in responses (Feature 4).
"""
...
async def list_dspm_findings(
    ...
    # Scope via DSPMFinding.cloud_account_id → CloudAccount.organization_id
    stmt = (
        select(DSPMFinding)
        .join(CloudAccount, DSPMFinding.cloud_account_id == CloudAccount.id, isouter=True)
    )
    ...
    return [
        {
            ...
            "cve_ids":               f.cve_ids,
            "cvss_max":              f.cvss_max,
            "vt_reputation":         f.vt_reputation,
            "threat_intel_boost":    f.threat_intel_boost,
            "threat_intel_enriched_at": f.threat_intel_enriched_at.isoformat() if f.threat_intel_enriched_at else None,
        }
        for f in rows
    ]


# ── DSPM Summary (org-scoped) ─────────────────────────────────────────────────
...
async def list_correlations(
    ...
    # Apply org scope to findings fetch
    f_stmt = (
        select(DSPMFinding)
        .join(CloudAccount, DSPMFinding.cloud_account_id == CloudAccount.id, isouter=True)
        .where(DSPMFinding.id.in_(finding_ids))
    )
    f_stmt = apply_org_scope(f_stmt, CloudAccount, scope)
    findings = {f.id: f for f in (await db.execute(f_stmt)).scalars().all()}
    ...
    return [
        ...
        for c in rows
        if c.dspm_finding_id in findings  # filter out findings outside user's org
    ]
